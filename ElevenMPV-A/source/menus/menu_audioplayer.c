...
void Menu_LoadExternalCover(void) {
	if (external_cover[0] != '\0' && metadata.cover_image == NULL && !ext_cover_loaded) {
		if (ext_cover_is_jpeg) {
			...
			metadata.cover_image = vita2d_load_JPEG_ARM_file(external_cover, VITA2D_IO_TYPE_NORMAL, 0, 0, 0);
			...
			metadata.cover_image = vita2d_load_PNG_file(external_cover, VITA2D_IO_TYPE_NORMAL);
			external_cover_tmp = metadata.cover_image;
			ext_cover_loaded = SCE_TRUE;
			metadata.has_meta = SCE_TRUE;
		}
...
void Menu_NotificationBegin(void) {
	...
	sceClibStrncpy((char *)init_param.text, "\xB9\xF8", 2);
	if (metadata.has_meta) {
		if (metadata.title[0] != '\0')
			Utils_Utf8ToUtf16(&init_param.text[1], metadata.title);
		else
			Utils_Utf8ToUtf16(&init_param.text[1], filename);
		if (metadata.artist[0] != '\0') {
			Utils_Utf8ToUtf16(init_param.subText, metadata.artist);
		}
	}
	else
		Utils_Utf8ToUtf16(&init_param.text[1], filename);
	...
	if (config.notify_mode == 1)
		Utils_Utf8ToUtf16(init_param.cancelText, "Stop playback?");
	else
		Utils_Utf8ToUtf16(init_param.cancelText, "Stop playback and close ElevenMPV-A?");

	init_param.cancelCallback = Utils_NotificationEventHandler;
...
void Menu_NotificationUpdate(void) {
	...
	if (paused) {
		sceClibStrncpy((char *)update_param.text, "\x9A\xF8", 2);
		if ((metadata.has_meta) && (metadata.title[0] != '\0'))
			Utils_Utf8ToUtf16(&update_param.text[1], metadata.title);
		else
			Utils_Utf8ToUtf16(&update_param.text[1], filename);
	}
	...
	Utils_Utf8ToUtf16(update_param.subText, time);

	update_param.progress = ((float)position / (float)length) * 100;
...
void Menu_PlayAudio(char *path) {
	...
	while(SCE_TRUE) {
		if (!sceKernelPollEventFlag(event_flag_uid, FLAG_ELEVENMPVA_IS_FG, SCE_KERNEL_EVF_WAITMODE_AND, NULL) && !isInSettings) {
			...
			if ((pressed & SCE_CTRL_L) || Touch_GetTapRecState(TOUCHREC_TAP_REW)) {
				if (count != 0)
					Music_HandleNext(SCE_FALSE, MUSIC_STATE_REPEAT_ALL);
			}
			else if ((pressed & SCE_CTRL_R) || Touch_GetTapRecState(TOUCHREC_TAP_FF)) {
				if (count != 0)

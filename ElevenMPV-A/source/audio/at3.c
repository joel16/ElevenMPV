...
static SceIoAsyncParam asyncParam = {0};
static _SceIoPreadAsyncParam preadParam = {0};
...
void AT3_Decode(void *buf, unsigned int length, void *userdata) {
	...
	if (callCount == 0) {
		...
		sceClibMemset(&asyncParam, 0, sizeof(SceIoAsyncParam));
		preadParam.offset = (SceOff)current_offset;
		preadParam.asyncParam = &asyncParam;
		bufindex ^= 1;
		switch (codecType) {
		case SCE_AUDIOCODEC_ATX:
			opHandle = _sceIoPreadAsync(file_fd, streamBuffer[bufindex], codecCtrl.atx.inputEsSize * GROUP_SIZE, &preadParam);
			break;
		case SCE_AUDIOCODEC_AT3:
			opHandle = _sceIoPreadAsync(file_fd, streamBuffer[bufindex], codecCtrl.inputEsSize * GROUP_SIZE, &preadParam);
			break;
		}
		bufindex ^= 1;
	}
